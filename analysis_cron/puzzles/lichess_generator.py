...
This is a reimplementation of the puzzle-generation algorithm used by
lichess (https://github.com/ornicar/lichess-puzzler, AGPL-3.0). It is written
from the published algorithm rather than copied, and is included here to bring
the same "only-move forced sequence" quality bar to this project.
...
* "Cook" a solution: a sequence where every winner move is the *only* good move
  (its winning chance clearly exceeds the second-best move's), and every
  defender move is the engine's best defense. Continue until the win is
  realized (advantage) or mate is delivered.

Winning chances use a logistic transform of the evaluation, matching lichess,
so that the "only move" test is meaningful near equality and saturates when
already winning.
"""
...
from chess.engine import Cp, Limit, Mate, PovScore, Score, SimpleEngine
...
@dataclass
class CookedPuzzle:
    # Board the solver faces (winner to move) and the full solution line.
    start_fen: str
    winner: Color
...
class LichessStyleGenerator:
    def __init__(self, engine: SimpleEngine, config: GeneratorConfig | None = None):
        self.engine = engine
        self.config = config or GeneratorConfig()

    # --- engine helpers -----------------------------------------------------
    def _limit(self) -> Limit:
        return Limit(depth=self.config.depth, time=self.config.time_sec, nodes=self.config.nodes)

    def _move_pair(self, board: chess.Board, winner: Color) -> MovePair:
        info = self.engine.analyse(board, multipv=2, limit=self._limit())
        best = info[0]
        ...
        return MovePair(
            ...
            winner=winner,
            best_move=best["pv"][0],
            best_score=best["score"].pov(winner),
            second_move=(second["pv"][0] if second else None),
            second_score=(second["score"].pov(winner) if second else None),
        )

    def _best_defense(self, board: chess.Board) -> Optional[Move]:
        result = self.engine.play(board, limit=self._limit())
        return result.move
    ...
    def _is_only_move(self, pair: MovePair) -> bool:
        ...
        if pair.best_score == Mate(1):
            # A mate in one is a valid single solution as long as the best
            # non-mating alternative isn't also comfortably winning.
    ...
    def _cook_mate(self, board: chess.Board, winner: Color) -> Optional[list[Move]]:
        ...
        if board.turn == winner:
            ...
            if pair.best_score < MATE_SOON:
                return None  # not actually a forced mate at this depth
            move = pair.best_move
        else:
    ...
    def _cook_advantage(self, board: chess.Board, winner: Color) -> Optional[list[MovePair]]:
        ...
        if board.turn == winner:
            ...
            if not self._is_only_move(pair):
                return []  # branch: no unique continuation, stop here
            if pair.best_score < Cp(200):
                return None  # advantage evaporated
            move = pair.best_move
            child = board.copy(stack=False)
            child.push(move)
            ...
            child = board.copy(stack=False)
            child.push(move)
            # Defender pairs carry the played move only.
    ...
    def _cook_from(
        ...
        if board.legal_moves.count() < 2:
            return None
        # Don't start from an already-winning position.
        ...
        if score > MATE_SOON:
            ...
            if not line:
                return None
            if len(line) < self.config.min_solution_plies:
                return None
        ...
        if score >= Cp(200) and win_chances(score) > win_chances(prev_score) + 0.6:
            # Require the advantage to be real: clearly winning, or earned from
            # not being up material (defensive/counterattacking resource).
            if score < Cp(400) and material_diff(board, winner) > -1:
                return None
            pairs = self._cook_advantage(board.copy(stack=False), winner)
            if not pairs:
                return None
            # Trim to end on a solver move that had a real alternative, keeping
            # the line odd-length (solver moves first and last).
    ...
    def analyze_game(
        ...
        puzzle starts; the engine is used to cook (and validate) the solution.
        """
        ...
        for node in game.mainline():
            ...
            if cooked is not None:
                return GamePuzzle(node=node, mistake_ply=ply, cooked=cooked, swing_cp=_swing_cp(prev_score, score))
