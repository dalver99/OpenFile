...
Uses python-chess's SVG board output, which browsers render natively, so no
Cairo/native image libraries are required. Solutions are hidden behind a
disclosure toggle so the page can be used for actual solving.
"""
...
def _card(index: int, puzzle: dict[str, Any]) -> str:
  ...
  <div class="meta">
    ...
    <div class="tags">Missed: {puzzle.get('cp_loss')} cp &middot; Quality: {puzzle.get('quality_score')} &middot; {html.escape(themes_str)}</div>
    <details>
